# Hi, I'm Haitao 👋

I invest in technology by day, and build it with a team of AI agents by night.

Investor at [Linear Capital](https://www.linear.vc). Programming and product-making are my lifelong hobbies — most of the green squares on this profile are joint work between me and my agents.

## Building now

- 🌍 **[gogoing](https://github.com/Openhaitao/gogoing)** — a spatial way to browse videos on iOS: content rises from behind a planet's horizon, drifts past you, and falls back below. Swift + RealityKit, zero dependencies.
- 💬 **[weixin-to-Raft](https://github.com/Openhaitao/weixin-to-Raft)** — talk to your entire AI agent team from WeChat: one bridge, `/agent` to switch anyone in, attachments both ways.
- 🔋 **[TokenCell](https://github.com/Openhaitao/TokenCell)** — a battery-style usage widget for Claude Code and Codex, so you always know how much juice is left.
- 🐰 **[NowNow](https://github.com/Openhaitao/NowNow)** — a shared "free paper" for small teams: Apple-Notes-style goal pages with the thinnest possible collaboration layer. A note first, a system second.
- 💭 **[ChatsHub](https://github.com/Openhaitao/ChatsHub)** — an early-stage chat product, being built in the open. One day, it might change the world.

## Find me

- 💬 WeChat: **ChatHHT** — always happy to talk about products, agents, and the ideas you can't stop thinking about
- 🏢 [Linear Capital](https://www.linear.vc)
